void nchar(char c, int n){
	while(n--){
		printf("%c", c);
	}
}

void printTab(){
	printf("\t\t\t\t\t");
}

int getInt(){
	int n;
	printf("\t\t\t:");
	scanf("%d", &n);
return n;
}

void topbar(){
	
	system("color 7");
	system("cls");
	printf("\n\n");
	printTab();
	
	
	printf("\n");
	printf("\t  _____                _     _                         _            _____    _                                                 _        \n");
  	printf("\t / ____|              | |   (_)                       | |          |  __ \\  | |                                               (_)       \n");
 	printf("\t| |  __    ___   ___  | |_   _    ___    _ __       __| |   ___    | |__) | | |__     __ _   _ __   _ __ ___     __ _    ___   _    ___ \n");
 	printf("\t| | |_ |  / _ \\ / __| | __| | |  /  _\\  |    _\\    / _  |  / _ \\   |  ___/  |  _ \\   / _  | |  __| |  _   _ \\   / _  |  / __| | |  / _ \\\n");
 	printf("\t| |__| | |  __/ \\__ \\ | |_  | | | (_) | | | | |   | (_| | |  __/   | |      | | | | | (_| | | |    | | | | | | | (_| | | (__  | | |  __/\n");
  	printf("\t \\_____|  \\___| |___/  \\__| |_|  \\___/  |_| |_|    \\__ _|  \\___|   |_|      |_| |_|  \\__ _| |_|    |_| |_| |_|  \\__ _|  \\___| |_|  \\___|\n");
                                                                                                                                         
	printf("\n\n\n\n");
}

void production(){
	system("color B");
	printf("\n\n\n\n\n");
			printf("\t\t|||||||||| ,ggg,     gg                                           ,gggg,                                \n");
			printf("\t\t||	dP""Y8a        88                                         ,88''''Y8b,                      8I          \n");
			printf("\t\t||	Yb, `88        88                                        d8'     `Y8                      8I          \n");
			printf("\t\t||	 `'  88        88                                       d8'   8b  d8                      8I          \n");
			printf("\t\t	     88        88                                      ,8I    'Y88P'                      8I          \n");
			printf("\t\t	     88        88    ,ggggg,    gg      gg             I8'             ,ggggg,      ,gggg,8I   ,ggg,  \n");
			printf("\t\t	     88       ,88   dP"  "Y8ggg I8      8I             d8             dP'  'Y8ggg  dP"  "Y8I  i8" "8i \n");
			printf("\t\t	     Y8b,___,d888  i8'    ,8I   I8,    ,8I             Y8,           i8'    ,8I   i8'    ,8I  I8, ,8I \n");
			printf("\t\t	      ''Y88888P''88,,d8,   ,d8'  ,d8b,  ,d8b,            `Yba,,_____, ,d8,   ,d8'  ,d8,   ,d8b, `YbadP' \n");
			printf("\t\t	           ,ad8888P'Y8888P'    8P''Y88P'`Y8              `'Y8888888 P'Y8888P'    P'Y8888P'`Y8888P'Y888\n");
			printf("\t\t	          d8P' 88                                                                                     \n");
			printf("\t\t	        ,d8'   88                                                                                     \n");
			printf("\t\t	        d8'    88                                                                                    || \n");
			printf("\t\t	        88     88                                                                                    || \n");
			printf("\t\t	        Y8,_ _,88                                                                                    || \n");
			printf("\t\t	         'Y888P'                                                                               |||||||| \n");
		sleep(5);
		system("cls");
		printf("\n\n\n");
		printf("\t\t			.---.               _        .-.   .---.  _          .--.  .--.  .--. \n");
		printf("\t\t			: .; :             :_;      .' `.  : .--':_;        : .--': .; :: .--'\n");
		printf("\t\t			:  _.'.--.  .--.   .-. .--. `. .'  : `;  .-.,-.,-.  `. `. :    :`. `. \n");
		printf("\t\t			: :   : ..'' .; :  : :' '_.' : :   : :   : :: ,. :   _`, :: :: : _`, :\n");
		printf("\t\t			:_;   :_;  `.__.'  : :`.__.' :_;   :_;   :_;:_;:_;  `.__.':_;:_;`.__.'\n");
		printf("\t\t			                 .-. :                                                     \n");                                                              
		printf("\t\t			                 `._.'                                                          \n");                                                         
		printf("\n\n\n");											
		printf("\t\t				.---.  .--. .---. .---.     .-..---.  .--. .---. .----.\n");
		printf("\t\t				`--. :: ,. :`--. :`--. :   .'.'`--. :: ,. :`--. :`--  ;\n");
		printf("\t\t				  ,',': :: :  ,','  ,','  .'.'   ,',': :: :  ,',' .' ' \n");
		printf("\t\t				.'.'_ : :; :.'.'_ .'.'_  .'.'  .'.'_ : :; :.'.'_  _`,`.\n");
		printf("\t\t				:____;`.__.':____;:____;:_:    :____;`.__.':____;`.__.'\n");
		
		sleep(5);
		}
		
void error404(){
	topbar();
	system("color 4");
	printf("\n\n\n\n\n\n\n");
printTab();	printf("  ______                                  _  _      ___    _  _   \n");
printTab();	printf(" |  ____|                                | || |    / _ \\  | || |  \n");
printTab();	printf(" | |__     _ __   _ __    ___    _ __    | || |_  | | | | | || |_ \n");
printTab();	printf(" |  __|   | '__| | '__|  / _ \\  | '__|   |__   _| | | | | |__   _|\n");
printTab();	printf(" | |____  | |    | |    | (_) | | |         | |   | |_| |    | |  \n");
printTab();	printf(" |______| |_|    |_|     \\___/  |_|         |_|    \\___/     |_|  \n");
	
	sleep(5);
	
	
}


